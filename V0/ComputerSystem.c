...
void ComputerSystem_PowerOn(int argc, char *argv[])
{
	...
	FILE *programNameFile, *programFile;
	...
	// Check for a custom program name
	if ((programNameFile = fopen(NAME_PROGRAM_TO_BE_EXECUTED, "r")) != NULL)
	{
		// Get the contents of the file
		char fileContents[LINEMAXIMUMLENGTH];
		fgets(fileContents, LINEMAXIMUMLENGTH, programNameFile);
		fclose(programNameFile);

		// Get only the first line without trailing spaces
		programFileName = strtok(fileContents, " *\n");
	}

	// If programFileName exists, is executed
	programFile = fopen(programFileName, "r");

	// Check if programFile exists, if not, default to the default name, if not again, poweroff system
	if (programFile == NULL)
	{
...
{
	ComputerSystem_DebugMessage(2, SHUTDOWN);
	exit(0);
}
...
void ComputerSystem_DebugMessage(int msgNo, char section, ...)
{
	...
	if ((strchr(debugLevel, ALL) != NULL) // Print the message because the user specified ALL
		...
		|| (strchr(debugLevel, section)) != NULL)
	{ //  or the section argument is included in the debugLevel string
